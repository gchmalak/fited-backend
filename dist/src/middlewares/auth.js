...
export function isAdmin(req, res, next) {
    if (req.user?.role === "admin" || req.user?.role === "owner") {
        ...
        errorResponse(res, "You are not an admin, you can't access this route", StatusCodes.FORBIDDEN);
    }
}
export function isOwner(req, res, next) {
    if (req.user?.role === "owner") {
        ...
        errorResponse(res, "Only the store owner can perform this action", StatusCodes.FORBIDDEN);
    }
}
