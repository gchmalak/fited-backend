...
export async function createContactMessage(req, res, next) {
    try {
        ...
        successResponse(res, contactMessage, "Message sent! We'll get back to you soon.", StatusCodes.CREATED);
    }
    catch (err) {
        next(err);
    }
...
export async function replyToContactMessage(req, res, next) {
    try {
        ...
        const originalMessage = contactMessage.messages.find((msg) => msg.sender === "customer")?.message ?? "";
      ...
      `,
        ...
        successResponse(res, contactMessage, "Reply sent!");
    }
    catch (err) {
        next(err);
    }
...
export async function getMyContactMessages(req, res, next) {
    try {
        const email = req.user?.email;
        if (!email) {
            errorResponse(res, "User email not found", StatusCodes.UNAUTHORIZED);
            return;
        }
...
export async function customerReplyToContactMessage(req, res, next) {
    try {
        const { id } = req.params;
        const { reply } = req.body;
        const email = req.user?.email;
        ...
        const contactMessage = await ContactMessage.findById(id);
        if (!contactMessage) {
            errorResponse(res, "Conversation not found", StatusCodes.NOT_FOUND);
            return;
        }
        // Make sure this conversation belongs to the logged-in customer
        if (contactMessage.email.toLowerCase() !== email.toLowerCase()) {
            errorResponse(res, "You are not allowed to reply to this conversation", StatusCodes.FORBIDDEN);
            return;
        }
